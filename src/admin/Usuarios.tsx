...
export default function Usuarios({ currentEmail }: { currentEmail: string | null }) {
  ...
  const saveRole = async () => {
    ...
    if (editing.email.toLowerCase() === OWNER_EMAIL && editRole !== 'admin') {
      setEditMsg('No se puede cambiar el rol del propietario.');
      return;
    }
  ...
  const createUser = async () => {
    ...
    if (email.toLowerCase() === OWNER_EMAIL && nRole !== 'admin') { setCreateMsg({ tone: 'err', text: 'El propietario siempre debe ser administrador.' }); return; }
          ...
          {users.map((u, i) => (
            <div key={u.id} className={`flex flex-wrap items-center gap-3 px-4 py-3.5 ${i > 0 ? 'border-t border-white/8' : ''}`}>
              ...
              {u.role === 'admin' ? (
                ...
                <Pill tone="cyan"><Bike className="h-3 w-3" /> Delivery</Pill>
              ) : (
        ...
        <div className="space-y-4">
          ...
          <Field label="Rol">
            <SelectInput value={editRole} onChange={(e) => setEditRole(e.target.value as Role)} disabled={savingRole}>
              ...
              <option value="delivery" className="bg-ink-800">Delivery (solo entregas)</option>
